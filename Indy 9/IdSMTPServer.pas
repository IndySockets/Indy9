...
{
{   Rev 1.5    3/16/04 11:13:32 AM  RLebeau
{ Buf fix for TIdSMTPGreeting.SetEHLOReply()
}
{
{   Rev 1.4    1/25/04 1:16:36 PM  RLebeau
{ Updated OnReceiveRaw event handling to seek the data stream to Position 0
{ prior to triggering the event.
}
{
        ...
        begin
          ...
          if FRawStreamType = stMemoryStream then begin
            ...
            try
              ...
              LStream.Position := 0;
              OnReceiveRaw(ASender, LStream, TIdSMTPServerThread(ASender.Thread).RCPTList, CustomError);
            finally
              FreeAndNil(LStream);
            end;
          end

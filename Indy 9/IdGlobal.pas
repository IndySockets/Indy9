...
{
    Rev 1.9    9/24/2004 12:42:46 AM  DSiders
  Modified GetTickCount to correct RangeOverflow error. (Reported by Mike
  Potter)
}
{
{   Rev 1.8    8/15/04 4:42:02 PM  RLebeau
{ Tweaks to AnsiPosIdx_() and PosIdx()
}
{
{   Rev 1.7    8/9/04 10:41:02 AM  RLebeau
{ Buf fix for TIdMimeTable.AddMimeType()
}
{
{   Rev 1.6    8/5/04 5:39:22 PM  RLebeau
...
function  AnsiPosIdx_ (const ASubStr: String; AStr: PChar; ALen: Cardinal; AStartPos: Cardinal = 0): Cardinal;
var
  LenSubStr: Cardinal;
  ByteType : TMbcsByteType;
  LStr, LSubStr, CurResult: PChar;
Begin
  Result := 0; //not found

  //*ALen := Length(AStr);
  LenSubStr := Length(ASubStr);
  if (LenSubStr = 0) or (LenSubStr > ALen) or (AStartPos > ALen) then begin
    Exit;
  end;

  LStr := Pointer(AStr);
  LSubStr := Pointer(ASubStr);

  //posIDX
  if AStartPos > 1 then begin
    Inc(LStr, AStartPos - 1);
    Dec(ALen, AStartPos - 1);
  end;//if
  if ALen <= 0 then begin
    Exit;
  end;

  CurResult := StrPos(LStr, LSubStr);
  while (CurResult <> nil) and ((ALen - Cardinal(CurResult - LStr)) >= LenSubStr) do begin //found and LenStr-Pos>=LenSubStr
    ByteType := StrByteType(LStr, Integer(CurResult-LStr));
{$IFDEF MSWINDOWS}
    if (ByteType <> mbTrailByte) and
      (Windows.CompareString(LOCALE_USER_DEFAULT, 0, CurResult, LenSubStr, LSubStr, LenSubStr) = 2) then begin
...
{$IFDEF LINUX}
    if (ByteType <> mbTrailByte) and
      (strncmp(CurResult, LSubStr, LenSubStr) = 0) then begin
      Result:=CurResult-Pointer(AStr)+1;
      Exit;
    end;//if
{$ENDIF}
    Inc(Result);
    CurResult := StrPos(CurResult, LSubStr);
  end;
End;//AnsiPosIdx

function  AnsiPosIdx(const ASubStr, AStr: String; AStartPos: Cardinal = 0): Cardinal;
Begin
